...
def getErrorCount():
    return max(len(duplicatesList), len(typeMismatchList),
               len(indexMismatchList), len(exceptionsList),
               len(colMismatchList), len(hasNansList))
...
for child in daily.iterdir():
    ...
    if df.index.dtype != 'datetime64[ns]':
        txt = indexMismatchText.format(child.name.upper().ljust(15),
                                       df.index.dtype)
    ...
    if colLength != 8:
        txt = columnMismatchText.format(child.name.upper().ljust(15), 5,
                                        colLength)
    ...
    for col in df.columns:
        if df[col].dtype not in ('float64', 'int64'):
            txt = dtypeMismatchText.format(child.name.upper().ljust(15), col,
                                           df[col].dtype)
