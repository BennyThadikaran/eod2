...
def lookup(sym):
    ...
    print(f'''{c.WHITE}Units represent average multiples. 1x 2x etc. < 1: below average.
DQ: Delivery qty  TQ: Qty per trade  IM: Institutional Money [Above average DQ and TQ]{c.ENDC}\n''')
    ...
    for Index, DQ, TQ, IM, VOL in df[::-1].itertuples():
        print(f'{c.CYAN}{Index.strftime("%d %b %Y").ljust(13)}{c.ENDC}',
...
group.add_argument('--ls',
                   action='store_true',
                   help='List available watchlists.')

group.add_argument('-l', '--lookup',
                   metavar='SYM',
                   help='Symbol to lookup')
